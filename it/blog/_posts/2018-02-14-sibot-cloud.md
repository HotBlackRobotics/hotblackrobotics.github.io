...
class Node(dotbot_ros.DotbotNode):
    ...
    def setup(self):
        ...
        motors_list = [90, 90, 90, 60, 90, 90, 20, 40, 120]
...
class Node(dotbot_ros.DotbotNode):
    ...
    def setup(self):
    # Definiamo due publisher e un subscriber per pubblicare le sequenze di motori, 
    # il valore per il gripper in gradi e sottoscriversi al topic su cui viene 
    # specificato se il gripper debba essere aperto o chiuso.
        self.pub = dotbot_ros.Publisher('motors_nointerp', Int16MultiArray)
        self.pubG = dotbot_ros.Publisher('gripper_value', String)
        dotbot_ros.Subscriber("gripper_value", String, self.gripper_callback)
    ...
    # di ogni elemento così da poter ciclare nella lista di valori per il gripper
        for indx, motors_seq in enumerate(self.m_list): 
    # Pubblichiamo la stringa letta da gripp_list, inducendo la chiamata 
    # della callback function gripper_callback, così da ottenere il valore 
    # in gradi per il gripper.
            self.pubG.publish(self.gripp_list[indx])
            seq = Int16MultiArray()
            seq.data = motors_seq
            seq.data.append(self.gripper)
    ...
    # unione degli angoli dei miniservo dei giunti e il valore per il gripper.
            self.pub.publish(seq)
            time.sleep(3)
# Funzione di callback quando viene ricevuto un messaggio sul topic gripper_value
    def gripper_callback(self,msg):
        if msg.data == 'open':
            self.gripper = 100
        else:
...
Abbiamo visto come controllare i valori dei motori di siBOT a spesso nella robotica industriale l'obbiettivo è controllare la posizione e l'orientamento dell'**End Effector** (**EE**) che in questo caso corrisponde al giunto della pinza (gripper)
<p align="center">
    <image src="/assets/imgs/2018-02-07-sibot-cloud/5_trigoreal.jpeg"  height="300"/>
</p>
Per poter ottenere la posizione desiderata è necessario manipolare questa informazione e trasformarla in valori per i motori del manipolatore i quali sono l'unico modo per muovere il robot. Questa operazione si chiama cinematica inversa e tramite calcoli, analitici o numerici, consente di trovare il valore dei motori corrispondenti ad una certa posizione dell'EE. Notate che in questo caso, possiamo definire solo la posizione desiderata (non l'orientamento) dal momento che la la pinza non ruota per configurazione fisica del braccio.
Anche in questo caso inviamo al braccio una sequenza di posizioni desiderate.
...
class Node(dotbot_ros.DotbotNode):
    ...
    def setup(self):
    # Definiamo un publisher per pubblicare le sequenze di posizioni per l'EE
        self.pub = dotbot_ros.Publisher('desired_position_nointerp', Point)
...
# in 20 steps (N=20)
    def motors_callback(self,data):
        ...
        if self.i == 1:
            ...
            if d.data[0] == 0 and d.data[1] == 0 and d.data[2] == 0:
...
// scrivi gli angoli sul rispettivo pin)
...
- Scrivere uno sketch chiamato *set_motors_sibot* che invii una sequenza per i motori ad ogni loop. 
- Scrivere uno sketch chiamato *set_position_sibot* che invii una posizione per l'EE ad ogni loop.
