...
La maggior parte degli esempi di Docker Container, per lo sviluppo di applicazioni, sono basati sull'installazione di nuovo software su, per esempio, Ubuntu, il quale non è realmente installato ma è rappresentato da contenuti del Flesystem necessari affinchè l'applicazione possa essere eseguita.
...
Il file che viene eseguito all'avvio può contenere operazioni di copia dall'Host al container, escuzione di altri script BASH ecc. Nel seguente file di esempio, utilizzato per un contenitore su cui è installato ROS, vengono eseguiti dei file di setup, viene avviato un server nginx e un launch file ROS.
```
!/usr/bin/env bash
set -e
...
il quale estrapola le informazioni di configurazione (mapping di porte, volumi, tag) da un file chiamato *docker-compose.yml* (di default) ed esegue il container con tutte le relative opzioni.
